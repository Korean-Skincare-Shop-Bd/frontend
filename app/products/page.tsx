...
import { Search, Filter, Grid, List, ShoppingBag, Eye, X, ChevronDown, Loader2 } from 'lucide-react';
  ...
  const FilterContent = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={`space-y-6 ${isMobile ? 'px-0' : ''}`}>
      ...
      <div className="space-y-3">
        ...
        >
          <CollapsibleTrigger className={`flex items-center justify-between w-full ${isMobile ? 'py-2' : ''}`}>
            <h3 className="font-semibold text-sm uppercase tracking-wider">Brands</h3>
            {isMobile && <ChevronDown className={`w-4 h-4 transition-transform ${expandedSections.brands ? 'rotate-180' : ''}`} />}
          </CollapsibleTrigger>
          <CollapsibleContent className="space-y-2">
            <div className={`space-y-2 ${isMobile ? 'mt-2' : ''} max-h-48 overflow-y-auto`}>
      ...
      <div className="space-y-3">
        ...
        >
          <CollapsibleTrigger className={`flex items-center justify-between w-full ${isMobile ? 'py-2' : ''}`}>
            ...
            {isMobile && <ChevronDown className={`w-4 h-4 transition-transform ${expandedSections.price ? 'rotate-180' : ''}`} />}
          </CollapsibleTrigger>
      ...
      {(searchQuery || selectedCategory !== 'all' || selectedBrand !== 'all' || priceRange[0] > 0 || priceRange[1] < 50000) && (
          ...
          <div className="flex lg:flex-row flex-col gap-6 lg:gap-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex sm:flex-row flex-col sm:justify-between sm:items-center gap-4 mb-4 sm:mb-6">
                ...
                {(searchQuery || selectedCategory !== 'all' || selectedBrand !== 'all' || priceRange[0] > 0 || priceRange[1] < 50000) && (
                  <Button variant="outline" size="sm" onClick={clearFilters} className="self-start sm:self-auto order-1 sm:order-2">
                    Clear Filters
                  </Button>
                )}
              </div>
              ...
              {loading ? (
                ...
                <div className="py-12 text-center">
                  <p className="mb-4 text-muted-foreground text-lg">No products found matching your criteria.</p>
                                  ...
                                  <div className="flex-1 space-y-1 sm:space-y-2 min-w-0">
                                    <div className="flex sm:flex-row flex-col sm:justify-between sm:items-start gap-2 sm:gap-4">
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-muted-foreground text-xs sm:text-sm truncate">{product.category?.name}</p>
                                      </div>
                                    ...
                                    {product.description && (
                                      <p className="hidden sm:block text-muted-foreground text-xs sm:text-sm line-clamp-2">
                                        {product.description}
                                      </p>
                                    )}
                                    ...
                                    <div className="flex sm:flex-row flex-col sm:justify-between sm:items-center gap-2 sm:gap-4 pt-1 sm:pt-2">
                                      ...
                                      <div className="flex sm:flex-row flex-col sm:items-center gap-2">
                                        {product.variations.length > 0 && (
                                          <span className={`text-xs px-2 py-1 rounded self-start ${product.variations[0].stockQuantity > 0
                                            ? 'bg-green-100 text-green-600'
                                            : 'bg-red-100 text-red-600'
                                            }`}>
                                            {product.variations[0].stockQuantity > 0 ? 'In Stock' : 'Out of Stock'}
                                          </span>
                                        )}
                    ...
                    <div className="flex sm:flex-row flex-col justify-center items-center gap-4 mt-8">
                      ...
                      <div className="hidden sm:flex justify-center items-center gap-2">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2">
                          ...
                          {pagination.totalPages > 5 && currentPage > 3 && currentPage < pagination.totalPages - 2 && (
                            <>
                              <span className="px-2 text-muted-foreground">...</span>
                          ...
                          {pagination.totalPages > 5 && currentPage >= pagination.totalPages - 2 && (
                            <>
                              <span className="px-2 text-muted-foreground">...</span>
                              {Array.from({ length: Math.min(3, pagination.totalPages) }, (_, i) => {
                                const pageNum = pagination.totalPages - 2 + i;
